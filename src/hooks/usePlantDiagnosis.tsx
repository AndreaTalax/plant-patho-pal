    ...
    try {
      ...
      if (conversations && conversations.length > 0 && conversations[0].conversation_type !== 'professional_quote') {
        ...
        if (pdfSent) {
          ...
          // Attendi un po' per assicurarsi che i dati siano salvati
          setTimeout(() => {
            console.log('🔄 Apertura chat dopo invio PDF');
            window.dispatchEvent(new CustomEvent('switchTab', { detail: 'chat' }));
          }, 1500);
        }
      } else if (conversations && conversations.length === 0) {
        console.log('📝 Nessuna conversazione attiva trovata, creo una nuova conversazione');
        ...
        const { data: newConversation, error: convError } = await supabase
          ...
          .insert({
            user_id: user.id,
            expert_id: '6ee6b888-8064-40a1-8b26-0658343f4360', // Marco Nigro ID
            title: `Consulenza - ${plant?.plantName || 'Pianta'}`,
            status: 'active'
          })
          .select()
          .single();
        ...
        if (!convError && newConversation) {
          console.log('✅ Nuova conversazione creata:', newConversation.id);
          ...
          if (pdfSent) {
            console.log('✅ PDF diagnosi inviato automaticamente alla nuova chat');
            toast.success('Chat creata e PDF inviato all\'esperto!');
